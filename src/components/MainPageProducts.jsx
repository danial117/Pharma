...
const MainPageProducts=()=>{
  ...
  if(select === 1){
   ...
   fetch(`http://localhost:3002/products?page=${page}&limit=6`,{
    method:'GET'
   }).then((response)=>response.json()).then((result)=>{setProducts(result); console.log(result); setPage(page+1)})
}
   else if(select ===2){
    ...
    const RequestProducts=()=>{
        ...
        fetch(`http://localhost:3002/products?page=${page}&limit=6`,{
    method:'GET'
     }).then((response)=>response.json()).then((result)=>{products.push(...result);setPage(page+1); console.log(products)})
    ...
    return(
       ...
       <div className="w-[30%] xs:max-lg:w-[60%] my-4 flex flex-row mx-auto">
           ...
           <div className="text-white bg-emerald-500 xs:max-sm:px-2 px-6 py-2">
            <SearchRounded style={{fontSize:isMobile?30 :40}} />
           </div>
